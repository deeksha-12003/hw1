---
title: "Homework 1"
author: "[Deeksha Banala]{style='background-color: yellow;'}"
toc: true
...
matrix_b <- matrix(c(1:100, (1:100)^2), nrow = 2, byrow = TRUE)
print(matrix_b)
...
I would expect the col_wise_scan to take lesser time to run. Since R stores data in matrices by column, the col_wise_scan works hand in hand with that while the row_Wise_scan goes against that order.
...
row_wise_time - Time difference of 0.009607077 secs

col_wise_time - Time difference of 0.0002138615 secs

We can clearly see that the row_wise_time was longer than the col_wise_time.
...
The time taken increases with the size of the matrix, with the column wise always taking lesser time than the row wise.
